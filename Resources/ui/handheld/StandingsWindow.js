...
Ti.API.info('rows.push '  + i);
    ...
    function createRow(rank, team, point, win, draw, lose, gotGoal, lostGoal, diffGoal, aclFlg) {
        ...
        Ti.API.info(team + '------------------------------------' + textColor);
        // 順位
        var rankLabel = createRowLabel(rank, 5, 20, 'center', textColor);
        row.add(rankLabel);
        var teamLabel = createRowLabel(team, 30, teamWidth, 'left', textColor);
        row.add(teamLabel);
        ...
        // 勝点
        var pointLabel = createRowLabel(point, leftPos, w2, "right", textColor);
        row.add(pointLabel);
        // 勝
        var winLabel = createRowLabel(win, leftPos+(w*1), w2, "right", textColor);
        row.add(winLabel);
        // 分
        var drawLabel = createRowLabel(draw, leftPos+(w*2), w2, "right", textColor);
        row.add(drawLabel);
        // 負
        var loseLabel = createRowLabel(lose, leftPos+(w*3), w2, "right", textColor);
        row.add(loseLabel);
        // 得
        var gotGoalLabel = createRowLabel(gotGoal, leftPos+(w*4), w2, "right", textColor);
        row.add(gotGoalLabel);
        // 失
        var lostGoalLabel = createRowLabel(lostGoal, leftPos+(w*5), w2, "right", textColor);
        row.add(lostGoalLabel);
        // 差
        var diffGoalLabel = createRowLabel(diffGoal, leftPos+(w*6), w2, "right", textColor);
     ...
     * @param {Object} width
     */
    function createRowLabel(text, left, width, textAlign, textColor) {
        if(!textAlign) {
            textAlign = 'right';
        }
